...
3. Ease of use: solving the equations of standard MCT takes very few lines of code, see the example below. While writted in `Julia`, the code can straightforwardly be called from `Python` and other languages. 
...
5.  Automatic differentiation: similarly, the use of dual numbers allow for forward-mode automatic differentiation. This enables, for example, the use of neural networks as surrogates for memory kernels or efficient methods for the solution of inverse problems.
