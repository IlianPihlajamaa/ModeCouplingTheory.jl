...
in which $\Omega^2_{\alpha\gamma} = k^2 k_B T x_\alpha/m_\alpha \cdot \left(S^{-1}\right)_{\alpha\gamma}(k)$, and $(S^{-1})_{\alpha\gamma}(k) = (\frac{\delta_{\alpha\gamma}}{x_\alpha} - \rho c_{\alpha\gamma}(k))$. The memory kernel is given by
